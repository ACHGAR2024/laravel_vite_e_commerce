                ...
                <form action="{{ route('produits.update', $produit) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input id="nom" name="nom" type="text" value="{{ $produit->nom }}"
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4"
                                class="p-5 mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input id="prix" name="prix" type="number" step="0.01" value="{{ $produit->prix }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                required>
                        </div>
                        <div>
                            ...
                            <input id="stock" name="stock" type="number" value="{{ $produit->stock }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                required>
                        </div>
                    </div>
                    <div>
                        ...
                        <select id="categorie_id" name="categorie_id"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @if ($produit->categorie_id == $category->id) selected @endif>
                                    {{ $category->nom }}</option>
                            @endforeach
                        </select>
                    </div>
