    ...
    <div x-data="{ activeSlide: 1 }" class="relative bg-transparent overflow-hidden">
        <div class="h-56 w-full object-cover sm:h-72 md:h-96 lg:w-full lg:h-full">
            ...
            <div x-show="activeSlide === 1" x-transition:enter="transition ease-out duration-300"
                ...
                style="background-image: url({{ asset('storage/images/images_home/' . $i . '.jpg') }})"></div>
        </div>
                    ...
                    <div class="sm:text-center lg:text-left bg-transparent bg-slate-200 bg-opacity-75 p-6 rounded-lg">
                        ...
                        <div class="mt-5 sm:mt-8 sm:flex sm:justify-center lg:justify-start">
                            <div class="rounded-md shadow">
                                <a href="{{ route('achat') }}"
                                    class="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 md:py-4 md:text-lg md:px-10 transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110">
                                    Acheter maintenant
                                </a>
                            </div>
                            <div class="mt-3 sm:mt-0 sm:ml-3">
                                <a href="{{ url('/en_savoir_plus') }}"
                                    class="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200 md:py-4 md:text-lg md:px-10 transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110">
                                    En savoir plus
                                </a>
                            </div>
                        </div>
                    ...
                    <div
                        ...
                        <div class="p-4">
                            ...
                            <form action="{{ route('panier.ajouter', $produit->id) }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="mt-4 w-full bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110">
                                    Ajouter au Panier
                                </button>
                            </form>
                        </div>
